import React, { useState } from "react";
import { Pause, Play } from "../../assets/svgs/svg-icons";

const Slider = () => {
  const [toggle, setToggle] = useState(true);
  return (
    <section className="w-full md:min-h-[45vh] pb-[4rem] relative bg-primary">
      <div className="main-container slider-statement relative">
        <marquee
          behaviour="alternate"
          scrollamount="15"
          className="slider-statement cursor-default py-[.5rem] bg-carpet-green relative"
        >
          <div className="element flex items-center gap-[5rem]">
            <span className="text-white font-serif text-[1.9rem] font-semibold">
              • Fresh Foods • Healthy Buy • From Farm 🚜 to Doorstep
            </span>
            <span className="text-white font-serif text-[1.9rem] font-semibold">
              • Fresh Foods • Healthy Buy • From Farm 🚜 to Doorstep
            </span>
            <span className="text-white font-serif text-[1.9rem] font-semibold">
              • Fresh Foods • Healthy Buy • From Farm 🚜 to Doorstep
            </span>
            <span className="text-white font-serif text-[1.9rem] font-semibold">
              • Fresh Foods • Healthy Buy • From Farm 🚜 to Doorstep
            </span>
            <span className="text-white font-serif text-[1.9rem] font-semibold">
              • Fresh Foods • Healthy Buy • From Farm 🚜 to Doorstep
            </span>
            <span className="text-white font-serif text-[1.9rem] font-semibold">
              • Fresh Foods • Healthy Buy • From Farm 🚜 to Doorstep
            </span>
            <span className="text-white font-serif text-[1.9rem] font-semibold">
              • Fresh Foods • Healthy Buy • From Farm 🚜 to Doorstep
            </span>
          </div>
        </marquee>
        <button
          onClick={() => setToggle(!toggle)}
          className="absolute top-0 right-[5%] flex items-center justify-center hover:opacity-[.9] transition-all delay-[.5sec] h-[2.75rem] w-[2.75rem] rounded-full bg-white border-solid border-[1.5px] border-gray-400 hover:border-gray-500"
        >
          {toggle ? <Pause /> : <Play />}
        </button>
      </div>
    </section>
  );
};

export default Slider;
